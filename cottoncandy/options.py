...
def generate_AES_key(bytes = 32):
	...
	try:
		...
		print('PyCrypto not installed. Reading from /dev/random instead')
...
# add things to old versions of config if needed
	...
	try:	# encryption section
		aesKey = config.get('encryption', 'key')
		if aesKey == 'auto':				# needs to generate a new key
			...
			config.set("encryption", 'key', 'cloud-encryption-key.txt')
			needs_update = True
	except configparser.NoSectionError:		# old config w/o encryption capabilities
